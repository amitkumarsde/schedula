...
- **Write a prescription.** After the visit time, add a diagnosis, medicines (name, dosage, duration) and instructions, then mark the visit completed. A diagnosis is optional — a simple visit can be finished with just instructions, or nothing. You can edit it later.
...
Only the files that are about the doctor.
...
│   ├── dashboard/components/DoctorDashboard.tsx      Numbers + appointment calendars
